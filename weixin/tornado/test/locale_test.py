...
from weixin.escape import utf8, to_unicode
from weixin.test.util import unittest, skipOnAppEngine
from weixin.util import u, unicode_type
...
class TranslationLoaderTest(unittest.TestCase):
    ...
    def clear_locale_cache(self):
        if hasattr(weixin.locale.Locale, '_cache'):
            del weixin.locale.Locale._cache

    def setUp(self):
        self.saved = {}
        for var in TranslationLoaderTest.SAVE_VARS:
            self.saved[var] = getattr(weixin.locale, var)
        self.clear_locale_cache()

    def tearDown(self):
        for k, v in self.saved.items():
            setattr(weixin.locale, k, v)
        self.clear_locale_cache()

    def test_csv(self):
        weixin.locale.load_translations(
            os.path.join(os.path.dirname(__file__), 'csv_translations'))
        locale = weixin.locale.get("fr_FR")
        self.assertTrue(isinstance(locale, weixin.locale.CSVLocale))
    ...
    def test_csv_bom(self):
        ...
        for encoding in ['utf-8-sig', 'utf-16']:
            ...
            try:
                ...
                weixin.locale.load_translations(tmpdir)
                locale = weixin.locale.get('fr_FR')
                self.assertIsInstance(locale, weixin.locale.CSVLocale)
                self.assertEqual(locale.translate("school"), u("\u00e9cole"))
            finally:
                shutil.rmtree(tmpdir)

    def test_gettext(self):
        weixin.locale.load_gettext_translations(
            os.path.join(os.path.dirname(__file__), 'gettext_translations'),
            "tornado_test")
        locale = weixin.locale.get("fr_FR")
        self.assertTrue(isinstance(locale, weixin.locale.GettextLocale))
        self.assertEqual(locale.translate("school"), u("\u00e9cole"))
...
class LocaleDataTest(unittest.TestCase):
    def test_non_ascii_name(self):
        name = weixin.locale.LOCALE_NAMES['es_LA']['name']
...
class EnglishTest(unittest.TestCase):
    def test_format_date(self):
        locale = weixin.locale.get('en_US')
