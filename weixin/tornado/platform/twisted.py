...
from weixin.concurrent import Future
from weixin.escape import utf8
from weixin import gen
import weixin.ioloop
from weixin.log import app_log
from weixin.netutil import Resolver
from weixin.stack_context import NullContext, wrap
from weixin.ioloop import IOLoop
from weixin.util import timedelta_to_seconds
...
class TornadoReactor(PosixReactorBase):
    ...
    def __init__(self, io_loop=None):
        if not io_loop:
            io_loop = weixin.ioloop.IOLoop.current()
...
class _FD(object):
    ...
    def doRead(self):
        if not self.lost:
            self.handler(self.fileobj, weixin.ioloop.IOLoop.READ)

    def doWrite(self):
        if not self.lost:
            self.handler(self.fileobj, weixin.ioloop.IOLoop.WRITE)

    def connectionLost(self, reason):
        if not self.lost:
            self.handler(self.fileobj, weixin.ioloop.IOLoop.ERROR)
            self.lost = True
...
class TwistedIOLoop(weixin.ioloop.IOLoop):
    ...
    def add_handler(self, fd, handler, events):
        ...
        if events & weixin.ioloop.IOLoop.READ:
            self.fds[fd].reading = True
            self.reactor.addReader(self.fds[fd])
        if events & weixin.ioloop.IOLoop.WRITE:
...
class TwistedResolver(Resolver):
    ...
    def initialize(self, io_loop=None):
        ...
        self.reactor = weixin.platform.twisted.TornadoReactor(io_loop)
