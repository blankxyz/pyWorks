...
代码历史:
    2014-2-25 : 葛洋,实现代码
    2014-5-5 : 葛洋,增加单个url外部接口
    2014-07-17：贺伟刚，增加salt参数
...
class Dedup():
    ...
    def __init__(self, con_str, key = "urlset", cache = None):
        ...
        self.key = key
        self.db = core.RedisDB("redis://%s"%con_str, key)
        if cache:
            self.cache = core.RedisDB("redis://%s"%cache[0],cache[1])
...
class MappingDedup():
    ...
    def __init__(self, regex_maps, default, cache=None):
        ...
        for regex_map in regex_maps:
            self.cons.append([re.compile(regex_map[0]), core.RedisDB("redis://%s"%regex_map[1], regex_map[2])])
                
        self.default = core.RedisDB("redis://%s"%default[0], default[1])
        if cache:
            self.cache = core.RedisDB("redis://%s"%cache[0],cache[1])
...
def main():
    dedup_mapping = (
        ("^http\://tieba\.baidu\.com/p/(\d+)$", "192.168.110.110:6379/0", "tieba_dedup"),
        ("^http\://weibo\.com/([\s\S]+)$", "192.168.110.120:6379/0", "sina_dedup"),
        ("^http\://t\.qq\.com/p/t/([\d]+)$", "192.168.110.120:6379/0", "qq_dedup"),
     )
    dedup_default = ("192.168.110.110:6379/0","dedup")
