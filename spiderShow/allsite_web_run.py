...
class MySqlDrive(object):
    ...
    def set_current_main_setting(self, start_url, site_domain, setting_json):
        ...
        try:
            ...
            print 'set_current_main_setting()', e, sqlStr1
            print 'set_current_main_setting()', e, sqlStr2
...
def setting_main():
    ...
    # 还原原有的正则表达式-详情页
    detail_rules = mysql_db.get_regexs('detail')
    i = 0
    for item in detail_rules:
        (black_domain, scope, white_or_black, weight, regex, etc) = item
    ...
    # 还原原有的正则表达式-列表页
    list_rules = mysql_db.get_regexs('list')
    i = 0
    for item in list_rules:
        (black_domain, scope, white_or_black, weight, regex, etc) = item
        regex_data = MultiDict([('select', True), ('regex', regex), ('score', int(0))])
        regexForm = RegexForm(regex_data)
        # inputForm.regex_list.append_entry(regexForm)
